 ...
 * This class handles some limitations of PHP\DateInterval and PHP\DateTime (e.g. format conversion, construction by designator)
 *
...
class TimeInterval {
    ...
    public function add($value) {
        $wto = new TimeInterval($value);
        $this->stamp += $wto->stamp;
        else $this->invert = $this->stamp < 0? 1 : 0;
    }

    public function sub($value) {
        $wto = new TimeInterval($value);
        $this->stamp -= $wto->stamp;
        $this->invert = $this->stamp < 0? 1 : 0;
    }
    ...
    */
     ...
     */
     protected function parse($spec) {
        ...
        $regex = "@(-)?P(\d+([.,]\d+)?Y)?(\d+([.,]\d+)?M)?(\d+([.,]\d+)?W)?(\d+([.,]\d+)?D)?(T(\d+([.,]\d+)?H)?(\d+([.,]\d+)?M)?(\d+([.,]\d+)?S)?)?@i"; // handles fractions

        if (!preg_match_all($regex, $spec, $matches) || strlen($matches[0][0]) != strlen($spec)) throw new Exception("Unknown or bad format ($spec)");

        $stamp = 0;
        $stamp += round(substr($matches[2][0], 0, -1) * 315360000); // get year - a year is interpreted as a year with 365 days
        $stamp += round(substr($matches[3][0], 0, -1) * 2592000); // get month - a month is interpreted as a month with 30 days
        $stamp += round(substr($matches[4][0], 0, -1) * 604800); // get weeks - each of 7 days
        $stamp += round(substr($matches[5][0], 0, -1) * 86400); // get days - each of 24 hours
        $stamp += round(substr($matches[7][0], 0, -1) * 3600); // get hours - each of 60 minutes
        $stamp += round(substr($matches[8][0], 0, -1) * 60); // get minutes - each of 60 seconds
        $stamp += round(substr($matches[9][0], 0, -1)); // get seconds - smallest unit
